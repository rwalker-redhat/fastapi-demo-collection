...
# Read
@router.get("/api/disney")
async def read_all_cartoon_characters(db: Session = Depends(get_db)):
    return db.query(models.CartoonCharacter).all()


@router.get("/api/disney/users")
async def read_all_by_user(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):

    if user is None:
        raise get_user_exception()
    return db.query(models.CartoonCharacter).filter(models.CartoonCharacter.owner_id == user.get("id")).all()


@router.get("/api/disney/read/{cartoon_character_id}/")
async def read_cartoon_character(cartoon_character_id: int,
    ...
    cartoon_character_model = db.query(models.CartoonCharacter)\
        ...
        .filter(models.CartoonCharacter.owner_id == user.get("id"))\
        .first()
...
async def create_cartoon_character(character: Character,
                                   user: dict = Depends(get_current_user),
                                   db: Session = Depends(get_db)):
    if user is None:
        raise get_user_exception()
    ...
    character_model.owner_id = user.get("id")
...
# Update - not filtering on owner_id!
@router.put("/api/disney/update/{cartoon_character_id}")
async def update_character(cartoon_character_id: int,
                           character: Character,
                           user: dict = Depends(get_current_user),
                           db: Session = Depends(get_db)):
...
# Delete
@router.delete("/api/disney/delete/{cartoon_character_id}")
async def delete_cartoon_character(cartoon_character_id: int,
                                   user: dict = Depends(get_current_user),
                                   db: Session = Depends(get_db)):
    if user is None:
        raise get_user_exception()
